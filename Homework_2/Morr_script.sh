...
read -p "Write the required number of lines for output (0-99): " lines
reg='^[0-9]{1,2}$' 
while [[ ! $lines =~ $reg ]]; do
...
while true; do
  read -p "Enable viewing additional connection states? (YES/NO)" yn
  case $yn in
  [Yy]*)
    add_states="y"
    break
...
function show_add_info {
  echo "Traveler, you can find other connection states in the Elder Scroll below:"
